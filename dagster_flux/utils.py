...
def prepare_match_to_llm(match: Match) -> dict[str, Any]:
    ...
    for team in match.match_teams:
        team_members = [(member.nickname, member.name, member.description) for member in team.members]
        match_to_llm["match_teams"].append((team.pretty_name, team.description, team_members))
